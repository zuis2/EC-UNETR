...
class SkipGatingUnit(nn.Module):
    def __init__(self, path_num, r=4): 
        super(SkipGatingUnit, self).__init__()

        self.global_avg_pool = nn.AdaptiveAvgPool3d(1)

        self.fc = nn.Sequential(
            nn.Linear(path_num, int(path_num * r)),  
            nn.ReLU(),          
            nn.Linear(int(path_num * r), path_num),     
            nn.Sigmoid(),  
        )

    def forward(self, x):
        x_ = x.transpose(0,2)
        x_ = self.global_avg_pool(x_)   
        x_ = x_.view(x_.size(0),x_.size(1),  -1)   

        gate = self.fc(x_)  

        c,b,p = gate.shape
        gate = torch.reshape(gate, (c,b, p, 1, 1, 1))
        gate = gate.transpose(0,2)
