...
def Conv_1(in_channels,out_channels,act_fn,ks=3,st=2,pa=1): #conv+batch+act
    model = nn.Sequential(
        nn.Conv3d(in_channels,out_channels, kernel_size=ks, stride=st, padding=pa),
        nn.BatchNorm3d(out_channels),
        act_fn,
    )
    return model

def Conv_2(in_channels,out_channels,act_fn,ks=3,st=2,pa=1,drp=0.2): #conv+batch+dropout+act
    model = nn.Sequential(
        nn.Conv3d(in_channels,out_channels, kernel_size=ks, stride=st, padding=pa),
        nn.BatchNorm3d(out_channels),
        nn.Dropout3d(p=drp),
...
def Conv_trans_1(in_channels,out_channels,act_fn,ks=3,st=2,pa=1,op=1): #trans_conv+batch+act
    model = nn.Sequential(
        nn.ConvTranspose3d(in_channels,out_channels, kernel_size=ks, stride=st, padding=pa,output_padding=op),
        nn.BatchNorm3d(out_channels),
        act_fn,
    )
    return model

def Conv_trans_2(in_channels,out_channels,act_fn,ks=3,st=2,pa=1,op=1,drp=0.2): #trans_conv+batch+dropout+act
    model = nn.Sequential(
        nn.ConvTranspose3d(in_channels,out_channels, kernel_size=ks, stride=st, padding=pa,output_padding=op),
        nn.BatchNorm3d(out_channels),
        nn.Dropout3d(p=drp),
...
def Conv_trans_3(in_channels,out_channels,act_fn,ks=3,st=2,pa=1,op=1): #trans_conv+act
    model = nn.Sequential(
        nn.ConvTranspose3d(in_channels,out_channels, kernel_size=ks, stride=st, padding=pa,output_padding=op),
        act_fn,
    )
...
class UnetGenerator_3d(nn.Module):
    ...
    def __init__(self,in_channels,out_channels,data_size,num_filter=12):
        ...
        self.down_1 = Conv_2(self.in_channels,self.num_filter*2,act_fn1)
        ...
        self.out = Conv_trans_3(self.num_filter*2,self.out_channels,act_fn2)
